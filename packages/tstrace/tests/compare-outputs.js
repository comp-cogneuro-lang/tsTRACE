...
exports.printComparisonResults = exports.compareWordOutputs = exports.compareCSVFiles = void 0;
 ...
 */
function compareCSVFiles(baselineFile, currentFile, options = {}) {
    ...
    if (!stats.rowCount.match) {
        errors.push(`Row count mismatch: baseline=${baselineLines.length}, current=${currentLines.length}`);
    }
    ...
    if (maxDiff > threshold) {
        errors.push(`Values exceed threshold: max_diff=${maxDiff.toExponential(3)}, threshold=${threshold.toExponential(3)}`);
    }
    return {
        file: path.basename(currentFile),
        status,
 ...
 */
function compareWordOutputs(baselineDir, currentDir, files = ['input.csv', 'feature.csv', 'phoneme.csv', 'word.csv']) {
    return files.map(file => compareCSVFiles(path.join(baselineDir, file), path.join(currentDir, file)));
}
exports.compareWordOutputs = compareWordOutputs;
/**
...
function printComparisonResults(results) {
    ...
    for (const result of results) {
        ...
        if (result.stats.numericStats.totalValues > 0) {
            ...
            console.log(`   max_diff: ${maxDiff.toExponential(3)}  mean_diff: ${meanDiff.toExponential(3)}`);
        }
...
exports.printComparisonResults = printComparisonResults;
//# sourceMappingURL=compare-outputs.js.map