...
var applyRestScaling = function (node, num) {
    if (node.RDL_rest_s) {
        return node.RDL_rest_s * Math.log10(node.RDL_rest_c * num);
    }
    return 0.0;
};
exports.applyRestScaling = applyRestScaling;
var applyWeightScaling = function (node, num, act_pi) {
    if (node.RDL_wt_s) {
        return act_pi * (1.0 + act_pi * node.RDL_wt_s * Math.log10(1.0 + num));
    }
    return act_pi;
};
exports.applyWeightScaling = applyWeightScaling;
var applyPostActivationScaling = function (node, num, exp_act_i) {
    if (node.RDL_post_c) {
        return exp_act_i * Math.log10(node.RDL_post_c + num);
    }
    return 0.0;
};
exports.applyPostActivationScaling = applyPostActivationScaling;
